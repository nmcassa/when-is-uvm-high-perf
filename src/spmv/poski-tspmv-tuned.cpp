...
//      default partition-matrix object {with #partitions = #threads} */
  ...
  int err = poski_TuneHint_MatMult (A_tunable, OP_TRANS, alpha, x_view, beta, y_view, ALWAYS_TUNE_AGGRESSIVELY);
  ...
  //  char* tuningis = oski_GetMatTransforms (A_tunable);
  
  std::cerr<<"tuned : "<<std::endl;

  util::timestamp tune_end;

  std::cerr<<"Tuning time: "<<tune_end-tune_begin<<std::endl;
