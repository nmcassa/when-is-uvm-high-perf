...
#pragma omp master
  ...
  //  std::cout<<nbrange<<std::endl;
  VertexType* coldeg = new VertexType[nVtx];
  
  for (VertexType i = 0; i < nVtx; ++i)
    coldeg[i] = 0;

  for (EdgeType j = 0; j < xadj[nVtx]; ++j)
    ++ coldeg [adj[j]];
  
  VertexType* part = new VertexType[nVtx+1];
  part[0] = 0;
  
  float step = ((float)xadj[nVtx]) / nbrange;

  for (int i = 0; i < nbrange; ++i) {
    VertexType cur = part[i]+1;
    EdgeType nnz = 0;
    while (nnz < step) {
      nnz += coldeg[cur];
      ++cur;
      if (cur >= nVtx) {cur = nVtx; break;}
    }
    part[i+1] = cur-1;
    //std::cout<<nnz<<" "<<part[i]<<std::endl; 
  }
