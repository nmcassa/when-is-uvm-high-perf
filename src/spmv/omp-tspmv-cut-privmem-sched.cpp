...
#pragma omp parallel
  ...
  {
    char* granstr = getenv("GRANULARITY");
    if (granstr != NULL) {
      granularity = atoi(granstr);
    }
  }
...
#pragma omp parallel // reduction(+ : count)
	...
	for (int colcnt = 0; colcnt < nbColumn; ++colcnt) {
	  int curcol = (colcnt + tid ) %nbColumn;

	  while (colptr[curcol] < colbegin[curcol+1]) {
	    
	    VertexType localbegin = __sync_fetch_and_add (&(colptr[curcol]) , granularity);
	    
	    VertexType localend = std::min(localbegin+granularity, colbegin[curcol+1]);

	    for (VertexType bl = localbegin; bl < localend; ++bl)
	    {
