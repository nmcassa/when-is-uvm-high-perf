...
void colorperm(int* bptrs, int* bids, int* colors, int nocolors, int n, int* lastcolor) {
  ...
  *lastcolor = 0;
  ...
  for(i = 0; i < nocolors; i++) {
    printf("batch %d has %d blocks CUTOFF %d lastcolor %d\n", i, bdegs[i+1], CUTOFF, *lastcolor);
    if(bdegs[i+1] >= CUTOFF) {
      *lastcolor = i+1;
    }
  }
...
{ 
  ...
  char* cutoffstr = getenv("CUTOFF");

  if (cutoffstr)
    CUTOFF = atoi (cutoffstr);
  else
    {
      CUTOFF = 122;
      std::cerr<<"CUTOFF undefined. defaulting to : "<<CUTOFF<<std::endl;
    }
 ...
 /* color the matrix rows */  
  ...
  printf("Colors permuted... lastcolor is %d\n", lastcolor); fflush(0);  
  ...
  int* rems = (int*)malloc(sizeof(int) * m);
  int noremaining = 0;
  for(int col = lastcolor; col < nocolors; col++) {    
...
#pragma omp for schedule(runtime)
	for (VertexType p = 0; p < noremaining; ++p)
	  {
	    VertexType i = rems[p];
	    Scalar output = 0.;
